...
def create_workflow_builder():
    # Create agents using credential-based Entra authentication
    researcher = AzureOpenAIChatClient(credential=_credential).as_agent(
        instructions=(
    ...
    marketer = AzureOpenAIChatClient(credential=_credential).as_agent(
        instructions=(
    ...
    legal = AzureOpenAIChatClient(credential=_credential).as_agent(
        instructions=(
