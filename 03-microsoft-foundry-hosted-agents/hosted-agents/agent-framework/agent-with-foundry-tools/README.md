...
The sample uses the rc2-compatible agent creation flow: `AzureOpenAIChatClient(...).as_agent(...)` with Entra credential authentication.
...
The sample reads environment variables from your shell session.
